...
class ACMS_POST_Form_Submit extends ACMS_POST_Form
{
    ...
    function sendAutoReply(& $Mail, & $Field)
    {
        ...
        if (isset($info['charset']) && strtolower($info['charset']) === 'iso-2022-jp') {
            Mailer::setEncoderIso2022Jp();
        }
        $mailer = Mailer::init();
    ...
    function getBaseMailInfo($Mail)
    {
        return array(
            ...
            'charset' => $Mail->get('Charset', 'UTF-8'),
            'charset-html' => $Mail->get('CharsetHTML', 'UTF-8'),
            'from' => '',
            'to' => array(),
            'subject' => '',
            'cc' => array(),
